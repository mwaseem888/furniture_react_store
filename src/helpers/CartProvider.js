...
export default function CartProvider(props) {
  ...
  // Add Product To Cart
  const addToCart = (item, quantity = 1, selectedSize = "Standard", selectedColor= "Standard") => {
    const index = cartItems.findIndex(
      itm => itm.id === item.id && itm.size === selectedSize && itm.color === selectedColor
    );
    if (index === -1) {
      const product = { ...item, cartQuantity: quantity, size: selectedSize, color: selectedColor};
  ...
  const updateQuantity = (item, delta) => {
    const copyCart = [...cartItems];
    const index = copyCart.findIndex(itm => itm.id === item.id && itm.size === item.size && itm.color === item.color);
  ...
  const removeFromCart = item => {
    setCartItems(
      cartItems.filter(e => {
        if(e.id === item.id)
          return e.size !== item.size ? true : false;
        else
          return true;
      }
    ))
